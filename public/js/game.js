...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const API_ENDPOINTS = {
    newQuestion:
      DATA_SOURCE_MODE === "CLOUDFLARE"
        ? "/api/new-question"
        : "/api/new-question-from-file",
    reviewQuestion:
      DATA_SOURCE_MODE === "CLOUDFLARE"
        ? "/api/review-question"
        : "/api/review-question-from-file",
  };
  ...
  function fetchAndDisplayQuestion() {
    ...
    fetch(
      `${API_ENDPOINTS.newQuestion}?lang=zh&difficulty=${currentDifficulty}`
    )
  ...
  function renderMainQuestion(data) {
    ...
    data.options.forEach((option) => {
      const button = createButton(option, () =>
        handleMainOptionClick(option.id)
      );
  ...
  function handleMainOptionClick(selectedId) {
    ...
    if (selectedId === correctId) {
      ...
      setTimeout(() => {
        // 在调用主函数前，先把UI重置为明确的加载状态
        resetUIForNewQuestion("这是哪节经文？", "正在出题...");
        fetchAndDisplayQuestion(); // fetchAndDisplayQuestion不再需要调用resetUI
      }, 1000);
    } else {
      correctStreak = 0;
      updateStreakDisplay();
  ...
  async function startReviewMode(verseIdToReview) {
    ...
    try {
      ...
      const response = await fetch(
        `${API_ENDPOINTS.reviewQuestion}?lang=zh&verseId=${verseIdToReview}`
      );
  ...
  function renderReviewQuestion(data) {
    ...
    data.options.forEach((option) => {
      const button = createButton(option, () =>
        handleReviewOptionClick(option.isCorrect)
      );
      optionsContainer.appendChild(button);
    });
  }
